...
def init_network(layers):
  ...
  for i in range(2, len(layers)+1):
    ...
    if layers[i]['type'] == 'Embed':
      params[i-1]['w'] = np.sqrt(3./(h*c))*np.random.randn(h*c, layers[i]['num']/3)
      params[i-1]['b'] = np.zeros(layers[i]['num'])
      h = 1
      c = layers[i]['num']
...
def network(params, layers, data, labels):
  ...
  for i in range(2, l):
    ...
    if layers[i]['type'] == 'Embed':
      output[i] = fully_connected.embedding_forward(output[i-1], layers[i], params[i-1])
    elif layers[i]['type'] == 'RELU':
  ...
  for i in range(l-1,1,-1):
    ...
    if layers[i]['type'] == 'Embed':
      output[i]['diff'] = input_back_deriv
      param_grad[i-1], input_back_deriv = fully_connected.embedding_backward(output[i],output[i-1],layers[i],params[i-1])
    elif layers[i]['type'] == 'RELU':
      output[i]['diff'] = input_back_deriv
...
def loss_func(w, b, X, y_true, labels, prediction):
  ...
  #print str(indices) + ":" + str(y_true)
  return obj, grad_w,grad_b, back_deriv, percent

def sgd_momentum(l_rate, params, param_grad):

  params_ = copy.deepcopy(params)

  for i in range(1,len(params_)+1):
    params_[i]['w'] = params[i]['w'] -  l_rate*param_grad[i]['w']
    params_[i]['b'] = params[i]['b'] -  l_rate*param_grad[i]['b']
